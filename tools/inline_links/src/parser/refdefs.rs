...
/// The label content cannot contain `]` or newline characters.
///
/// Example: "[my label]" -> "my label"
fn parse_link_label(input: &str) -> IResult<&str, &str> {
    delimited(
        char('['),
...
/// This can be a URL enclosed in angle brackets `<>`, or a raw URL.
/// For simplicity, this parser handles URLs without spaces or newlines.
///
/// Example: "<http://example.com/path>" -> "http://example.com/path"
/// Example: "http://example.com/path" -> "http://example.com/path"
fn parse_link_destination(input: &str) -> IResult<&str, &str> {
    alt((
        // Destination enclosed in angle brackets
        delimited(
            char('<'),
            take_while1(|c: char| c != '>' && c != '\n' && c != '\r'),
            char('>'),
        ),
        // Raw destination (any non-whitespace, non-control characters)
...
fn parse_link_title(input: &str) -> IResult<&str, &str> {
    alt((
        // Double quotes
        delimited(
            char('"'),
