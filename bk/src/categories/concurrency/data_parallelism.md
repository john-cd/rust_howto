...
Convert calls to `iter` or `iter_mut` or `into_iter` into `par_iter` or [`par_iter_mut`][par_iter_mut]⮳{{hi:par_iter_mut}} or `into_par_iter` to execute in parallel{{hi:Parallel execution}}.
