...
An mdbook pre-processor that controls visibility of private chapters and sections within them
...
A pre-processor for mdbook that adds support for hidden chapters
...
A pre-processor for mdbook to add inline Table of Contents support.
...
A pre-processor and a backend to config theme for mdbook, especially creating a pagetoc on the right and setting full color themes from the official ace editor.
...
A `mdbook` pre-processor to run arbitrary commands.
</div>
