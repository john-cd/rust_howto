...
| Parsing | `nom`, `pest`, [`lalrpop`][c-lalrpop]⮳{{hi:lalrpop}}, [`combine`][c-combine]⮳{{hi:combine}} | These crates offer different approaches to parsing, from combinators ([`nom`][c-nom]⮳{{hi:nom}}, [`combine`][c-combine]⮳{{hi:combine}}) to parser generators ([`pest`][c-pest]⮳{{hi:pest}}, [`lalrpop`][c-lalrpop]⮳{{hi:lalrpop}}). |
| Serialization/Deserialization | [`serde`][c-serde]⮳{{hi:serde}}, [`serde_json`][c-serde_json]⮳{{hi:serde_json}}, [`serde_yaml`][c-serde_yaml]⮳{{hi:serde_yaml}}, [`toml`][c-toml]⮳{{hi:toml}} | [`serde`][c-serde]⮳{{hi:serde}} is a powerful framework for serialization and deserialization, often used with text-based formats like JSON, YAML, and TOML. |
