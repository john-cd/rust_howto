...
Gets the current working directory{{hi:Current working directory}} by calling [`std::env::current_dir`][c~std::env::current_dir~docs]{{hi:std::env::current_dir}}↗ then for each entries in [`std::fs::read_dir`][c~std::fs::read_dir~docs]{{hi:std::fs::read_dir}}↗ extracts the
[`std::fs::DirEntry::path`][c~std::fs::DirEntry::path~docs]{{hi:std::fs::DirEntry::path}}↗ and gets the metadata via [`std::fs::Metadata`][c~std::fs::Metadata~docs]{{hi:std::fs::Metadata}}↗. The
