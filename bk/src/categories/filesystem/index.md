...
For most common file system tasks, [`std::fs`](https://doc.rust-lang.org/std/fs/index.html)↗{{hi:std::fs}} and [`std::path`](https://doc.rust-lang.org/std/path/index.html)↗{{hi:std::path}} will be sufficient. [`fs_extra`][c~fs_extra~docs]↗{{hi:fs_extra}} is useful for more advanced operations. [`tempfile`][c~tempfile~docs]↗{{hi:tempfile}} is great for working with temporary files. [`glob`][c~glob~docs]↗{{hi:glob}} simplifies file pattern matching. Use [`notify`][c~notify~docs]↗{{hi:notify}} for file watching.

| Topic | Rust Crates or Modules |
|---|---|
| File I/O | [`std::fs`](https://doc.rust-lang.org/std/fs/index.html)↗ (standard library) provides the core functionality for working with files and directories (opening, reading, writing, creating, deleting, etc.). |
| Paths | [`std::path`](https://doc.rust-lang.org/std/path/index.html)↗ (standard library) provides types and functions for working with file paths ([`Path`](https://doc.rust-lang.org/std/path/struct.Path.html)↗{{hi:Path}}, [`PathBuf`](https://doc.rust-lang.org/std/path/struct.PathBuf.html)↗{{hi:PathBuf}}), extracting file names, extensions, and other path components. |
| Directories | [`std::fs`](https://doc.rust-lang.org/std/fs/index.html)↗ includes functions for creating, listing, and traversing directories. |
| File Metadata | [`std::fs`](https://doc.rust-lang.org/std/fs/index.html)↗ allows you to retrieve information about files (size, modification time, permissions, etc.). |
...
| Symbolic Links | [`std::fs`](https://doc.rust-lang.org/std/fs/index.html)↗ supports working with symbolic links (though this is platform-dependent). |
