...
[`dirs`][c~dirs~docs]↗{{hi:dirs}} is a low-level library that provides platform-specific standard locations of directories for [[config | configuration]], cache and other data by leveraging the mechanisms defined by the XDG base/user directory specifications on Linux, the "Known Folder" API on [[Windows]], and the Standard Directory guidelines on macOS.
