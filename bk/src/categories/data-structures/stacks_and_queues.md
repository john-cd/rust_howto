...
While Rust's standard library doesn't have dedicated `Stack`{{hi:Stack}} and `Queue`{{hi:Queue}} types in the same way some other languages do, you can easily use existing data structures instead - primarily [`Vec`][c~std::vec::Vec~docs]↗ for stack-like behavior, and [`VecDeque`][c~std::collections::VecDeque~docs]↗ for queue-like behavior.
...
```rust,editable
{{#include ../../../crates/cats/data_structures/examples/stacks_and_queues/stack.rs:example}}
```
...
```rust,editable
{{#include ../../../crates/cats/data_structures/examples/stacks_and_queues/queue.rs:example}}
```
