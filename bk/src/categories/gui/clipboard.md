...
Some GUI frameworks, like `iced` or `egui`, might have their own clipboard integration, but often they use the underlying clipboard crate.

### WebAssembly (WASM) Considerations

Clipboard access in WASM is restricted due to security reasons. You'll typically need to use JavaScript APIs to interact with the clipboard in a WASM context. Crates like `wasm-bindgen` would be used for this JavaScript interop.
