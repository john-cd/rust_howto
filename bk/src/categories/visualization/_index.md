...
| Data Visualization Libraries (General) | `iced`, `egui`, `tauri` (for desktop apps), `yew` (for web apps) | These crates provide frameworks for building applications that often include data visualization components. `iced` and `egui` are immediate mode GUIs. `tauri` and `yew` are for building desktop and web applications, respectively. |
...
| Graph Visualization | `petgraph`, `dot` (for graph description language) | `petgraph` is a graph library that can be used with visualization tools. `dot` can be used to generate graph descriptions in the DOT language, which can then be rendered by Graphviz. |
| Image Processing & Visualization | `image`, `imageproc` | These crates are used for image processing and manipulation, which can be part of a visualization pipeline. |
...
| Data Exploration & Analysis (Related) | `polars`, `dataframe` | These crates are useful for data manipulation and analysis, often a precursor to visualization. |
...
- Graph Visualization: `petgraph` with graph rendering tools
