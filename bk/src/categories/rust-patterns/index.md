...
| | `tokio` Asynchronous Runtime | Building asynchronous applications using futures, tasks, and actors. | `tokio` crate documentation |
...
| Interior Mutability | `std::cell`, `std::sync::atomic` | `Cell`, `RefCell`, `Atomic*` types are in the standard library. |
| Error Handling | | `Result` and `Option` are part of the prelude. Crates like `anyhow` and `thiserror` can help with error management. |
| Generics & Traits | | Fundamental language features. |
| Concurrency | `std::thread`, `std::sync`, `crossbeam`, `tokio`, `async-std` | `std::thread` and `std::sync` provide basic threading and synchronization primitives. `crossbeam` offers more advanced concurrent data structures. `tokio` and `async-std` are asynchronous runtimes. |
| Builder | Often implemented directly, no specific crate | While often implemented directly, the `derive_builder` crate can help generate boilerplate for the builder pattern. |
...
| Observer | `event-listener` | Helps implement the observer pattern. |
| Dependency Injection | `di`, `yew` for front-end | Dependency injection frameworks exist, but it's often handled manually, especially in smaller projects. `yew` uses dependency injection for its component system. |
| Closures | | Fundamental language features. |
| Iterators | | Fundamental language features, but the `itertools` crate provides additional iterator adapters. |
| Macros | | Fundamental language features. Crates like `syn` and `quote` are used for procedural macros. |
...
| Serialization Serde | `serde`, `serde_json`, `serde_yaml`, `toml` | `serde` is a powerful framework for serialization and deserialization. |
...
| Testing | Built-in - no specific crates | Rust has built-in support for unit and integration testing. Crates like `rstest` can help with testing. |
