...
[`async-graphql`][c-async_graphql]⮳{{hi:async-graphql}} is a high-performance graphql server library that's fully specification compliant. It integrates with [`actix-web`][c-actix_web]⮳{{hi:actix-web}}, [`axum`][c-axum]⮳{{hi:axum}}, `poem`, [`rocket`][c-rocket]⮳{{hi:rocket}}, `tide`, and [`warp`][c-warp]⮳{{hi:warp}}.
