...
Also note that, if we use `pub` before a struct definition, we make the struct public, but the struct’s fields will still be private. Mark relevant fields with `pub`:
