...
| [Amazon][amazon-website]⮳ | Amazon Web Services has adopted Rust for certain infrastructure, networking, and [machine [learning][p-learning]][p-machine-learning] services, including their [serverless][p-serverless] computing - see, for example, [Amazon Firecracker][firecracker-microvm-website]⮳. |
...
| [Autumn Engineering][autumnai-github]⮳ | Offers [machine [learning][p-learning]][p-machine-learning] frameworks [written in Rust][p-written-in-rust]. |
...
| [Bitfury][bitfury-website]⮳ | Exonum is an extensible framework for blockchain projects [written in Rust][p-written-in-rust]. |
| [Block][block-website]⮳ | |
...
| [Calyptech][calyptech-website]⮳ | Used for high [performance][p-performance] [embedded][p-embedded] system components as an alternative to C. |
| [Canonical][canonical-website]⮳ | Everything from [server][p-server] monitoring to [middleware][p-middleware]! |
| [CarGurus][cargurus-website]⮳ | |
| [Cash App][cash-app-website]⮳ | |
| [Ceph][ceph-website]⮳ | Provides an open-source storage system partially [written in Rust][p-written-in-rust]. Rust bindings for librbd, an interface into the Ceph storage platform. |
| [Chef][chef-website]⮳[(github)][chef-github]⮳ | Offers a platform to configure, deploy, and manage applications, [written in Rust][p-written-in-rust]. |
| [Chroma][chroma-website]⮳[(github)][chroma-github]⮳ | |
| [Clever [Cloud][p-cloud]][clever-cloud-website]⮳ | Allow developers to host web applications. A part of their infrastructure is developed in Rust. |
| [Cloudflare Workers][cloudflare-workers-website]⮳ | Deploy [serverless][p-serverless] code across the globe with Rust support. |
...
| [Craft][craft-website]⮳ | Their core [machine [learning][p-learning]][p-machine-learning] engine is [written in Rust][p-written-in-rust]. |
| [Cryptape][cryptape-website]⮳ | Their business intelligence platform built for blockchain is [written in Rust][p-written-in-rust]. |
| [CurrySoftware GmbH][currysoftware-github]⮳ | |
| [DataRobot][datarobot-website]⮳[(github)][algorithmia-github]⮳ | Builds [Machine [Learning][p-learning]][p-machine-learning] models that are implemented using Rust. |
...
| [Discord][discord-website]⮳ | Uses Rust for both the client and [server][p-server] sides of its codebase, including their real-time communication services. |
...
| [EVO][evo-website]⮳ | Develops marketplaces for online shopping, fintech, and logistics. [Development tools][p-development-tools], containerization, monitoring and orchestration systems [written in Rust][p-written-in-rust]. |
| [Embark Studios][embark-studios-website]⮳ | |
| [Embecosm][embecosm-website]⮳ | |
| [Espressif][espressif-website]⮳[(github)][espressif-github]⮳ | Improving [performance][p-performance] of [embedded][p-embedded] and IoT devices with using Rust in esp products. |
| [Exonum][exonum-website]⮳ | Supports smart contracts [written in Rust][p-written-in-rust]. |
...
| [Ferrous Systems][ferrous-systems-website]⮳[(github)][ferrous-systems-github]⮳ | Ferrous is a Rust consulting firm specializing in [embedded][p-embedded] systems development. |
| [Figma][figma-website]⮳ | Uses Rust for parts of its design and collaboration tools. Their real-time multiplayer syncing [server][p-server] (used to edit all Figma documents) is [written in Rust][p-written-in-rust]. |
...
| [Hive][thehive-website]⮳ | Offers pre-trained AI models for automating content moderation. Hive 2.0 is [written in Rust][p-written-in-rust]. |
...
| [Meta][meta-website]⮳ | Uses Rust for the source control backend, Libra (now Diem), and other server-side projects. Facebook's primary source control system is partially [written in Rust][p-written-in-rust]. |
| [Microsoft][microsoft-website]⮳ | Uses Rust for various projects, including the [Windows][p-windows] kernel, Azure, and Visual Studio. [`windows-rs`][windows-rs]⮳{{hi:windows-rs}} allows you to call Windows API using Rust. There are several Azure services also using Rust, including [Azure IoT Edge][azure-iot-edge-github]⮳ and Kusto, the core query and storage engine for [Azure Data Explorer][azure-data-explorer]⮳. |
...
| [Npm][npm-website]⮳[(github)][npm-github]⮳ | The package manager for JavaScript has components [written in Rust][p-written-in-rust]. Also use Rust for their authorization service. |
...
| [OneSignal][onesignal-website]⮳[(blog)][onesignal-blog]⮳ | High volume, [cross platform][p-cross-platform] push notification delivery. |
...
| [Pollen [Robotics][p-robotics]][pollen-robotics-website]⮳ | |
...
| [Salesforce ][salesforce-website]⮳ | Uses Rust & [WASM][p-wasm] for MuleSoft's Flex Gateway, a new gateway running on Envoy. |
| [SandboxVR][sandboxvr-website]⮳ | |
| [Sandstorm][sandstorm-website]⮳ | The backend of their Collections app is [written in Rust][p-written-in-rust]. |
| [Scythe Robotics][scytherobotics-website]⮳ | |
| [Sensirion][sensirion-website]⮳ | |
| [Sentry][sentry-website]⮳ | Uses Rust for its performance-critical components. JavaScript, [Java][p-java] and iOS event processing and the command-line client for the Sentry API. |
| [Shopify][shopify-website]⮳ | |
| [Slint][c-slint-website]⮳[(github)][c-slint-github]⮳ | |
| [Slowtec][slowtec-website]⮳ | Their automation systems are entirely [written in Rust][p-written-in-rust]. |
| [SmartThings (Samsung)][smartthings-website]⮳ | Memory-safe [embedded][p-embedded] applications on our SmartThings Hub and supporting services in the [cloud][p-cloud]. |
...
| [Starlab][starlab-website]⮳ | Provides [embedded][p-embedded] security. |
...
| [System76][system76-website]⮳[(github)][system76-github]⮳ | Manufacturer of Linux computers and keyboards. As a Linux-based computer-manufacture, much of their infrastructure and desktop [Linux][p-linux] projects are [written in Rust][p-written-in-rust]. |
| [Tag1 Consulting][tag1consulting-website]⮳ | |
| [Tailcall][tailcall-website]⮳[(github)][tailcallhq-github]⮳ | A [cloud][p-cloud] native solution to streamline API management across edge, middle, and service layers. |
| [Tarro][tarro-website]⮳ | |
...
| [Tweede Golf][tweedegolf-website]⮳[(github)][tweedegolf-github]⮳ | Create safe internet infrastructure with Rust and secure connected devices with [Embedded][p-embedded] Rust. |
