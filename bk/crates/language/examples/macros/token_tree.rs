...
fn main() {
    // A "token tree" can a single lexical token (identifier, Rust keyword, operator, punctuation, or literal):
    accept_single_token_tree!("a");
    accept_single_token_tree!( :: );
    accept_single_token_tree!( , );
    accept_single_token_tree!( for );

    // A token tree can be a list of tokens within balanced delimiters `()`, `[]`, or `{}`:
    ...
    // Use `$( $name:tt )+` for one or more token trees; `$( $name:tt );*` for semicolon-separated tokens, etc.
}
// ANCHOR_END: example
