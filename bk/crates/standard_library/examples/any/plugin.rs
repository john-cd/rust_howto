...
// pub trait Plugin: Send + Sync + 'static { // `'static` is required for `Any`
...
// use plugin_interface::{Plugin, PluginContext, AsAny};
// use std::{
//     any::{Any, TypeId},
//     collections::HashMap,
//     sync::{Arc, Mutex},
...
//         self.shared_data.get(&type_id)
//             .and_then(|boxed_any| boxed_any.downcast_ref::<T>())
//     }

//     fn get_data_mut<T: Any + Send + Sync>(&mut self) -> Option<&mut T> {
//         let type_id = TypeId::of::<T>();
//         self.shared_data.get_mut(&type_id)
