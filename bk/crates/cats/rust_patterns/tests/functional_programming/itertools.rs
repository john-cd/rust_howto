// ANCHOR: example
//! This example demonstrates the usage of various functions provided by the `itertools` crate.
...
fn main() {
    ...
    // `cloned` creates an iterator that clones the elements of the original iterator.
    //  Here, it's used to clone the bytes of the byte string "abc" and check if the first element is 'a'.
    assert_eq!(itertools::cloned(b"abc").next(), Some(b'a'));

    // `dedup` removes consecutive duplicate elements from an iterator.
    // Here, it removes consecutive duplicates from the vector [1., 1., 2., 3., 3., 2., 2.]
    // and asserts that the result is [1., 2., 3., 2.].
    let data = vec![1., 1., 2., 3., 3., 2., 2.];
    itertools::assert_equal(data.into_iter().dedup(), vec![1., 2., 3., 2.]);

    // `into_group_map` groups elements of an iterator into a map based on a key.
