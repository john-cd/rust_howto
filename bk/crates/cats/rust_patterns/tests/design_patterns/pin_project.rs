...
//! `Pin`-ned struct.
//!
//! Pinning is a mechanism that guarantees that the memory location of a value
//! will not change. This is useful when there are one or more pointers pointing
//! at that value, for example in self-referential data structures,
...
///
/// - For the fields that use #[pin] attribute, it creates the pinned reference
///   to the field.
...
/// `#[pin_project]` also implements the following methods on the original type:
...
struct SelfReferential {
    ...
    // In this example, we rely upon pinning for soundness and must ensure that
    // the type is not automatically marked as `Unpin`. We do so by adding a
    // `PhantomPinned` marker field.
...
/// not move out of any structurally pinned fields.
///
...
impl<T: Debug> PinnedDrop for PinnedWithDrop<T> {
    fn drop(self: Pin<&mut Self>) {
        ...
        println!(
            "Dropping PinnedWithDrop with pinned_field {:?} and unpinned_field: {}",
            self.pinned_field, self.unpinned_field
        );
    }
}
