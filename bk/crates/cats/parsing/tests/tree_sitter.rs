...
//     // Parse the code
//     let tree = parser.parse(code, None).ok_or(anyhow::anyhow!("Error parsing
// the code"))?;
...
// query.capture_index_for_name("variable").unwrap() {     //         let node =
// capture.node;     //         let start = node.start_byte();
...
//     // println!("\nFunction names:");
//     // for m in query_cursor_fn.matches(&query_fn_name, tree.root_node(),
// code.as_bytes()) {     //     // Each match contains the index of the pattern
// that matched, and a list of captures.     //     let capture = m.captures[0];
...
//         }
//     "#;
//     let tree_with_predicate = parser.parse(code_with_predicate,
// None).ok_or(anyhow::anyhow!("Error parsing the code with predicate"))?;;

//     // println!("\nBinary expressions where the left side is 'x':");
//     // for m in query_cursor_predicate
//     //     .matches(&query_predicate, tree_with_predicate.root_node()
...
// [right_node.start_byte()..right_node.end_byte()];     //     println!("- {} +
