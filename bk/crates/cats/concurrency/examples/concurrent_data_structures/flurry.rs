...
fn main() {
    ...
    let map = Arc::new(HashMap::new());
    {
        ...
        // We obtain a `Guard` using `HashMap::guard` and can
        // use references to the same guard to make one or more API
        // calls.
        let guard = map.guard();
        map.insert("key0".to_string(), 0, &guard);
        // Keep in mind that for as long as we hold onto this `Guard`, we are
    ...
    }
    {
        ...
        // Keep in mind that for as long as we hold onto this, we are
        // preventing the collection of garbage generated by the map.

        // We can also re-use a map pin like so:
        let mref = map.pin();
