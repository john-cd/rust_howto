...
Runs `git log --oneline` as an external [`std::process::Command`][c~std::process::Command~docs]{{hi:std::process::Command}}⮳ and inspects its [`std::process::Output`][c~std::process::Output~docs]{{hi:std::process::Output}}⮳ using [`regex::Regex`][c~regex::Regex~docs]{{hi:regex::Regex}}⮳ to get the hash and message of the last 5 commits.
