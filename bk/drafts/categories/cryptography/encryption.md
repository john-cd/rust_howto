...
[![rsa][c~rsa~docs~badge]][c~rsa~docs] [![rsa~crates.io][c~rsa~crates.io~badge]][c~rsa~crates.io] [![rsa~github][c~rsa~github~badge]][c~rsa~github] [![rsa~lib.rs][c~rsa~lib.rs~badge]][c~rsa~lib.rs]{{hi:rsa}}{{hi:Crypto}}{{hi:Security}}{{hi:Encryption}}{{hi:rsa}} [![cat~cryptography][cat~cryptography~badge]][cat~cryptography]{{hi:Cryptography}}
...
[`sodiumoxide`][c~sodiumoxide~docs]⮳{{hi:sodiumoxide}} is a fast cryptographic library for Rust (bindings to [`libsodium`][c~libsodium~docs]⮳{{hi:libsodium}}). It implements encryption, decryption, signatures, password hashing, and more.
