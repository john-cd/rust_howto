...
| Security | [`cargo-audit`][c~cargo-audit~docs]↗{{hi:cargo-audit}} checks whether any of your dependencies are of a version that has a security advisory out against them. [`cargo-geiger`][c~cargo-geiger~docs]↗{{hi:cargo-geiger}} identifies dependencies with unsafe code, so we can either audit them or find alternatives. |
...
<div class="hidden">
[review in depth, reorg table](https://github.com/john-cd/rust_howto/issues/311)

- [cargo-prefetch][c~cargo-prefetch~repo]↗: Cargo subcommand to download popular crates.
