...
| Static Site Generation (SSG) | [`perseus`][c~perseus~docs]↗{{hi:perseus}} (based on [`sycamore`][c~sycamore~crates.io]↗{{hi:sycamore}}), [`zola`][zola~website]↗{{hi:zola}} | These tools generate static websites from templates and content. |
...
| [[network-programming | Networking]] (Low-Level) | [`tokio::net`](https://docs.rs/tokio/latest/tokio/net)↗{{hi:tokio::net}}, [`std::net`][c~std::net~docs]↗{{hi:std::net}} | These modules provide low-level networking primitives. Often used by higher-level frameworks. |
