...
[`tinysearch`][c~tinysearch~docs]↗{{hi:tinysearch}} is compiled to WebAssembly to run in a browser. It can be used together with static site generators such as `Jekyll`, `Hugo`, [`Zola`][zola~website]↗{{hi:Zola}}, [`Cobalt`](https://cobalt-org.github.io)↗{{hi:Cobalt}}, or `Pelican`.
