...
- [`kondo`: Cleans dependencies and build artifacts from your projects.][kondo~github]↗
- [`cargo-sweep`: A cargo subcommand for cleaning up unused build files generated by Cargo][c~cargo-sweep~github]↗
- [`cargo-update`: A cargo subcommand for checking and applying updates to installed executables][c~cargo-update~github]↗
- [`cargo-xtask`][c~cargo-xtask~github]↗
- [`cargo-script`][c~cargo-script~crates.io]↗
- [`cargo-play`][c~cargo-play~crates.io]↗
- [`sccache-action`: `sccache` github action][sccache-action~github]↗.
...
- [`cargo-bloat`: Find out what takes most of the space in your executable.][c~cargo-bloat~github]↗.
...
- [`watchexec`: Executes commands in response to file modifications][c~watchexec~github]↗.
...
- [`vscode-just`: VSCode syntax highlighting for just files][vscode-just~github]↗.

- [`dotbot`: A tool that bootstraps your dotfiles][dotbot~github]↗.

- [Bazel at Enterprise Scale | BuildBuddy][buildbuddy~website]↗.

- [The Pants Build System][pants~github]↗.

- [`mini-docker-rust`: Very small rust docker image][mini-docker-rust~github]↗.
