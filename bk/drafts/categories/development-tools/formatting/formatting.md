...
| Topic | Rust Crates |
|---|---|
| Code Formatter | [`rustfmt`][c~rustfmt~github]↗{{hi:rustfmt}} is the standard Rust code formatter. |
| Formatting Configuration | [Configuration][p~configuration] is done through `rustfmt.toml` files. |
| Integration | Most [IDEs][p~ides] have built-in [`rustfmt`][c~rustfmt~github]↗{{hi:rustfmt}} integration. |
...
Install [`rustfmt`][c~rustfmt~github]↗{{hi:rustfmt}} with `rustup component add rustfmt`
...
Using `--check` instructs [`rustfmt`][c~rustfmt~github]↗{{hi:rustfmt}}{{hi:rustfmt}} to exit with an error code if the input is not formatted correctly (useful for CI).
...
For things you do not want [`rustfmt`][c~rustfmt~github]↗{{hi:rustfmt}} to mangle, use `#[rustfmt::skip]`{{hi:rustfmt::skip}} , `#[rustfmt::skip::macros(macro_name)]`{{hi:rustfmt::skip::macros(macro_name)}} , or `#![rustfmt::skip::attributes(custom_attribute)]`{{hi:rustfmt::skip::attributes(custom_attribute)}}
