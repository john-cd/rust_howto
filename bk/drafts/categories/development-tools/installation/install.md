...
| Installer Generators | [`cargo-deb`][c~cargo_deb~docs]↗{{hi:cargo-deb}}, [`cargo-rpm`][c~cargo_rpm~docs]↗{{hi:cargo-rpm}}, [`create-dmg`](https://github.com/create-dmg/create-dmg)↗{{hi:create-dmg}} (for creating Debian packages, RPM packages, and [macOS][p~macos] disk [images][p~images], respectively) |
