...
## Advanced version of what is in `justfile`.
## Uses a custom builder.
## The GitHub Action CI workflow uses the GitHub Actions cache, which is not supported with the default `docker` build driver.
