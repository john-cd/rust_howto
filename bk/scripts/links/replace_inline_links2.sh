...
do
  ...
  do
    ...
    if [[ -z "$label" ]]; then
        ...
        sed -n -E 's<(https://doc.rust-lang.org/stable/cargo/commands/)([^.]*)(\.html)<[book~cargo~\2]: \1\2\3<p' <<< "${in_parens}"

        # https://www.reddit.com/r/rust/comments/1fyown4/rust_gpu_the_future_of_gpu_programming
        sed -n -E 's<(https://www.reddit.com/r/rust/comments/)([^/]+/)(.*)<[reddit~\3]: \1\2\3<p' <<< "${in_parens}"

        # https://doc.rust-lang.org/std/cmp/index.html
        sed -n -E 's<(https://doc.rust-lang.org/std/)(.*)<[c~std::\2]: \1\2<p' <<< "${in_parens}"
