...
- [`GitHub Actions`]( ){{hi: }} is triggered by the push or pull request.
...
- [`GitHub Actions`]( ){{hi: }} creates a new release on GitHub, attaching the packaged binary.
- Deployment (Optional): [`GitHub Actions`]( ){{hi: }} deploys the application to a target environment.
