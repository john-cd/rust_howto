...
fn test_replace_multiple_types() {
    ...
    let expected = "[![clap][c~clap~docs~badge]][c~clap~docs]{{hi:clap}} [![regex][c~regex~docs~badge]][c~regex~docs]{{hi:regex}} [![walkdir][c~walkdir~github~badge]][c~walkdir~github]{{hi:walkdir}}";
    assert_eq!(replace(text), expected);
}
...
fn test_replace_in_file_with_match() -> Result<()> {
    ...
    assert_eq!(
        contents,
        "This is a test file with [![clap][c~clap~docs~badge]][c~clap~docs]{{hi:clap}}.\n"
    );
    dir.close()?;
    Ok(())
}
