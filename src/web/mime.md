...
filename using the [mime][mime] crate.  The program will check for file extensions
and match against a known list.  The return value is [`mime:Mime`][mime:Mime]
...
When receiving a HTTP reponse from `reqwest` the [MIME type][MIME type] or media type may be
found in the [Content-Type][Content-Type] header. [`reqwest::header::HeaderMap::get`][reqwest::header::HeaderMap::get] retrieves
