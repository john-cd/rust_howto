# Functions

[![Rust by example - Functions][book-rust-by-example-fn-badge]][book-rust-by-example-fn]{{hi:fn}}{{hi:Functions}}

```rust
{{#include ../../deps/tests/lang/functions.rs}}
```

The unit type{{hi:Unit type}} `()`{{hi:()}} (`void` in some languages) is the default return type{{hi:Return type}} when no type is given for a function. It could be omitted: `fn log(message: &str) { ... }`

## Generic functions

```rust
{{#include ../../deps/tests/lang/generic_functions.rs}}
```

```rust
{{#include ../../deps/tests/lang/generic_functions2.rs}}
```

## Function pointers

```rust
{{#include ../../deps/tests/lang/function_pointers.rs}}
```

## Diverging functions

{{i:Diverging functions}} never return.

```rust,should_panic
{{#include ../../deps/tests/lang/diverging_functions.rs}}
```

{{#include ../refs/link-refs.md}}
<div class="hidden">
TODO: add desc
</div>
