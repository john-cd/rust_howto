...
Rust will never automatically create “deep” copies of your data. Use [`{{i:clone}}`][c-std::clone::Clone]⮳

```rust,editable
{{#include ../../deps/tests/clone.rs}}
```

If a type implements the [`{{i:Copy}}`][c-std::marker::Copy]⮳ trait (stack-only, fixed-size values, like integers, floats, and tuples thereof), variables that use it do not move, but rather are trivially copied, making them still valid after assignment to another variable.
