...
```rust,editable
{{#include ../../crates/ex/standard_library/tests/cow/cow1.rs:example}}
```

Use [`std::convert::AsRef`][c-std::convert::AsRef]{{hi:std::convert::AsRef}}⮳:

```rust,editable
{{#include ../../crates/ex/standard_library/tests/cow/cow2.rs:example}}
```

Use [`std::ops::Deref`][c-std::ops::Deref]{{hi:std::ops::Deref}}⮳ explicitly:

```rust,editable
{{#include ../../crates/ex/standard_library/tests/cow/cow3.rs:example}}
```

Use [`std::ops::Deref`][c-std::ops::Deref]{{hi:std::ops::Deref}}⮳ implicitly through a coercion{{hi:Coercion}}:

```rust,editable
{{#include ../../crates/ex/standard_library/tests/cow/cow4.rs:example}}
```
