...
The `Box<T>` type is a {{i:smart pointer}} because it implements the {{i:`Deref`}} trait, which allows `Box<T>` values to be treated like references. Implementing the `Deref` trait allows you to customize the behavior of the {{i:dereference operator}} {{i:`*`}}.
