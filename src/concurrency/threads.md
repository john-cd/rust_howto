...
`[crossbeam_channel::Receiver::iter]` method will block, either waiting for new messages or until the channel is closed. Because the channels were created within the [`crossbeam::scope`][crossbeam::scope] we must manually close them via `drop` to prevent the entire program from blocking on the worker for-loops. You can think of the calls to `drop` as signaling that no more messages will be sent.
...
`[crossbeam::scope]` and [`Scope::spawn`][Scope::spawn] to manage the producer thread. Data is exchanged between the two threads using a `[crossbeam_channel::unbounded]`
...
current directory. A threadpool generates threads equal to the number of cores present in the system found with `[num-cpus::get]`. [`Walkdir::new`][Walkdir::new] iterates the current directory and calls [`execute`][execute] to perform the operations of reading
...
Create [`ThreadPool`][ThreadPool] with thread count equal to number of cores with `[num-cpus::get]`.
`[ThreadPool::execute]` receives each pixel as a separate job.

`[mpsc::channel]` receives the jobs and [`Receiver::recv`][Receiver::recv] retrieves them.
