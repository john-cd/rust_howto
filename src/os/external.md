...
Opens the `python` interpreter using an external [`Command`][Command] and passes it a
python statement for execution. [`Output`][Output] of statement is then parsed.
...
processing doesn't start until external [`Command`][Command] is finished.
The recipe below calls [`Stdio::piped`][Stdio::piped] to create a pipe, and reads
