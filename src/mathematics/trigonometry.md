...
By default, Rust provides mathematical [float methods][float methods] such as
...
points on the Earth with the [Haversine formula][Haversine formula]. Points are expressed
as pairs of latitude and longitude in degrees. Then, [`to_radians`][to_radians]
converts them in radian. [`sin`][sin] [`cos`][cos] [`powi`][powi] and [`sqrt`][f64-sqrt]
