...
Clone the [repo][rust-howto-github]⮳ and open the folder in [VS Code][rust-in-vs-code]⮳. Edit `.devcontainer/.env` if needed. {{i:VS Code}} should prompt you to open the code in a [`{{i:docker}}`][docker]⮳ container, which installs [`{{i:mdbook}}`][mdbook-documentation]⮳ and {{i:rust tooling}} automatically. Make sure you have previously installed
