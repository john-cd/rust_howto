...
- Add {{i:Rust code examples}} under [`deps/tests`][rust-howto-deps-tests-github] (or sometimes under `deps/examples`).
...
- The {{i:Rust playground}} only supports the top 100 most downloaded libraries and libraries used by the Rust Cookbook. `noplayground` removes the play button if a code block does not work on the playground.
