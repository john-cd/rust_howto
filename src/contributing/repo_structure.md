...
- After the book is built using {{hi:mdbook}}[`mdbook`][c-mdbook-documentation]⮳, the resulting HTML and Javascript are found in `book/html`.
- The intermediate (processed) Markdown is in `book/markdown`. The {{hi:mdbook}}[`mdbook`][c-mdbook-documentation]⮳ configuration is in `book.toml`; the templates and assets are in `theme` and `static` respectively.
- The Rust code is organized as a {{hi:cargo}}[`cargo`][c-cargo]⮳ workspace:
