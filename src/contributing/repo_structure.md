...
- After the book is built using [`{{i:mdbook}}`][c-mdbook-documentation]⮳, the resulting HTML and Javascript are found in `book/html`.
- The intermediate (processed) Markdown is in `book/markdown`. The [`{{i:mdbook}}`][c-mdbook-documentation]⮳ configuration is in `book.toml`; the templates and assets are in `theme` and `static` respectively.
- The Rust code is organized as a [`{{i:cargo}}`][c-cargo]⮳ workspace:
