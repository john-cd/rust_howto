...
These examples calculate measures of central tendency for a data set contained within a Rust array. There may be no mean, median or mode to calculate for an empty set of data, so each function returns an [`{{i:Option}}`][c-std::option::Option]⮳ to be handled by the caller.

The first example calculates the mean (the sum of all measurements divided by the number of measurements in the set) by producing an iterator of references over the data, and using [`{{i:sum}}`][c-std::iter::Iterator::sum]⮳ and [`{{i:len}}`][primitive-slice::len]⮳ to determine the total value and count of values respectively.
...
The final example calculates the mode using a mutable [`{{i:HashMap}}`][c-std::collections::HashMap]⮳ to collect counts of each distinct integer from the set, using a [`{{i:fold}}`][c-std::iter::Iterator::fold]⮳ and the [`{{i:entry}}`][c-std::collections::hash_map::Entry]⮳ API. The most frequent value in the [`{{i:HashMap}}`][c-std::collections::HashMap]⮳ surfaces with [`{{i:max_by_key}}`][c-std::iter::Iterator::max_by_key]⮳.
