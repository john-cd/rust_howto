...
- [`lld`][lld-website]⮳ on Windows and Linux, a linker developed by the LLVM project;
- [`zld`][zld-github]⮳ on MacOS.
...
[`mold`][mold-github]⮳ is up to 5× faster than [`lld`][lld-website]⮳, but with a few caveats like limited platform support and occasional stability issues. To install `mold`, run `sudo apt-get install mold clang` in Ubuntu.

You will also need to add the following to your [`cargo`][cargo]⮳ config at `.cargo/config.toml`:
