...
The Rust compiler spends a lot of time in the "link" step. LLD is much faster at {{i:linking}} than the default Rust linker.
...
You will also need to add the following to your [`{{i:cargo}}`][cargo]⮳ config at `.cargo/config.toml`:
