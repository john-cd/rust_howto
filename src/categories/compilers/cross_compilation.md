...
[`{{i:cross}}`][cross-github]⮳ builds your Rust project for different target operating systems and architectures. It requires [`{{i:rustup}}`][rustup-website]⮳ and [`{{i:Docker}}`][docker]⮳ or [`{{i:Podman}}`][podman]⮳.
...
[`{{i:cross}}`][cross]⮳ has the exact same CLI as [`{{i:cargo}}`][cargo]⮳ but relies on [`{{i:docker}}`][docker]⮳ or [`{{i:podman}}`][podman]⮳.
