...
The Rust standard library provides smart pointer types, such as `Mutex{{hi:Mutex}}<T>` and `Arc{{hi:Arc}}<T>`, that are safe to use in concurrent contexts.
