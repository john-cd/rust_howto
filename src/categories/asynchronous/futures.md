...
The {{hi:futures}}[`futures`][c-futures]⮳ crate provides a number of core abstractions for writing {{i:asynchronous code}}.
...
{{hi:Select}}[`Select`][c-futures::future::Select]⮳ polls multiple futures and streams simultaneously, executing the branch for the future that finishes first. If multiple futures are ready, one will be pseudo-randomly selected at runtime.
...
The {{hi:futures}}[`futures`][c-futures]⮳ crate provides an extension trait that provides a variety of convenient adapters.
