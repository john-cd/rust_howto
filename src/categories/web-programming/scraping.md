...
Iterates through {{i:links}} in the document and creates a [`{{i:tokio::spawn}}`][c-tokio::task::spawn]⮳ task that will parse an individual link with [`{{i:url::ParseOptions}}`][c-url::ParseOptions]⮳ and [`{{i:Url::parse}}`][c-tokio::task::spawn]⮳. The task makes a request to the links with [`{{i:reqwest}}`][reqwest]⮳ and verifies
