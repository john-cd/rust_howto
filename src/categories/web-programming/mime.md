...
The following example shows how to parse a [`{{i:MIME}}`][mime::Mime] type from a string using the [mime][mime]⮳ crate. [`{{i:FromStrError}}`][mime::FromStrError]⮳ produces a default [`{{i:MIME}}`][mime::Mime]⮳ type in an [`{{i:unwrap_or}}`][std::result::Result::unwrap_or]⮳ clause.
...
The following example shows how to return the correct {{i:MIME}} type from a given filename using the [`{{i:mime}}`][mime]⮳ crate. The program will check for file extensions and match against a known list. The return value is [`{{i:mime:Mime}}`][mime::Mime]⮳.
