...
[`{{i:rustup}}`][rustup-website]⮳ is a toolchain multiplexer. It installs and manages many Rust toolchains and presents them all through a single set of tools installed to `~/.cargo/bin`. The [`{{i:rustc}}`][rustc]⮳ and [`{{i:cargo}}`][c-cargo]⮳ executables installed e.g. in `~/.cargo/bin` are proxies that delegate to the real toolchain.
