...
Reads standard CSV records into [`{{i:csv::StringRecord}}`][csv::StringRecord]⮳ — a weakly typed data representation which expects valid UTF-8 rows. Alternatively,
[`{{i:csv::ByteRecord}}`][csv::ByteRecord]⮳ makes no assumptions about UTF-8.

```rust,editable
{{#include ../../../deps/tests/read.rs}}
```

[`{{i:serde}}`][serde]⮳ deserializes data into strongly type structures. See the [`{{i:csv::Reader::deserialize}}`][csv::Reader::deserialize]⮳ method.
...
Reads CSV records with a tab [`{{i:delimiter}}`][csv::ReaderBuilder::delimiter]⮳.
...
CSV files often contain {{i:invalid data}}. For these cases, the [`{{i:csv}}`][csv]⮳ crate provides a {{i:custom deserializer}}, [`{{i:csv::invalid_option}}`][csv::invalid_option]⮳ which automatically converts invalid data to [`{{i:None}}`][std::option::Option::None]⮳ values.
...
This example shows how to serialize a Rust tuple. [`{{i:csv::writer}}`][csv::writer]⮳ supports automatic {{i:serialization}} from Rust types into CSV records. [`{{i:write_record}}`][csv::Writer::write_record]⮳ writes a simple record containing string data only. Data with more complex values such as numbers, floats, and options use [`{{i:serialize}}`][csv::Writer::serialize]⮳. Since CSV writer uses an internal buffer, always explicitly [`{{i:flush}}`][csv::Writer::flush]⮳ when done.
...
See [`{{i:csv::Reader::deserialize}}`][csv::Reader::deserialize]⮳, [`{{i:serde::Deserialize}}`][serde::Deserialize]⮳ and [`{{i:std::str::FromStr}}`][std::str::FromStr]⮳.
