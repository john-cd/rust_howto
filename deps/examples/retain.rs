...
// fn parse_response(response: reqwest::blocking::Response) ->
// Result<u32> {     let mut body = response.text()?;
//     body.pop();
...
fn main() {
    ...
    // error),         ErrorKind::Reqwest(_) => println!("Reqwest
    // error: {:?}", error),         ErrorKind::ParseIntError(_)
